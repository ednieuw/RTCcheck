/* 
 ...
 Changes V012: Equal to ESP32_WordClockV150. Added RTCcheck V011 
 Changes V013: NUMLED 1 -> 14 LEDS. snprintf in SWversion() Timestamp/min ||hour in menu
 Changes V014: Print time /min /ht /off K2 K1 K0
 Changes V015: Equal to ESP32Arduino_WordClockV153.ino
 Changes V016: Added DoNotLog flag. Removed action="/get" <form id="commandForm" action="/get">
               changed to char* :-> PrintRTCTime());PrintNTPtime());PrintUTCtime()); PrintDS3231Time());
 Changes V017: Double printing of menu in Logging solved. Option K2 debugged. Equal to ESP32_WordClockV159
 Changes V018: Cosmetic.
 Changes V019: Cosmetic. Equal to ESP32Arduino_WordClockV160.ino
 Changes V020: Stable 
 Changes V021: Added option U in menu 
 Changes V022: Ringbuffer as in woordclockV183
 Changes V023: Cleaning up and cosmetics.  
 Changes V024: Ringbuffer long log file as download
 Changes V025: Temp -10C _ 60C -> 15C _ 25C
 Changes V026: Added CheckWIFIcommand() to avoid WIFI crashes when a command take several seconds to end, like LEDtest()
 Changes V026: changed delimiters in time log. Temp  15C _ 25C -> 10C - 30C and color calculation  LED10.
*********************
How to compile: 
...
Pin Numbering: By Arduino pin (default)     -- > with EdsoftLED both can be used
               ...
               when using By Arduino pin Error: Adafruit_NeoPixel/esp.c:78:(.text.espShow+0x55): undefined reference to `digitalPinToGPIONumber' 
...
const uint32_t  NUM_LEDS = 10;  // Used strip is 14 LEDs long. Will now clear all the LEDs at startup
...
#define TCA_ADDR 0x70                 // RTC8                                                // TCA9548A I2C multiplexer adres (standaard 0x70)
#define MAX_RTC 8                     // RTC8                                                // Maximaal Noof RTC modules
uint32_t RTCdiffSec[MAX_RTC];         // RTC8                                                // Seconds deviating from NTP time
...
struct    EEPROMstorage {                                                                     // Data storage in EEPROM to maintain them after power loss
  ...
  byte ByteFuture1      = 0;                                                                  // 
  byte TimeMinute       = 1;                                                                  // Display time per hour  
  byte DCF77On          = 0;                                                                  // Use DCF77
...
void EverySecondCheck(void)
{
 ...
 if (msLeap > 999)                                                                             // Every second enter the loop
 {
  ...
  // SetTempColor(8, timeinfo.tm_sec/2);    
...
{   
 ...
 NoTextInLeds = ff;   
 DimLeds(false);                                                                              // 
 if(timeinfo.tm_hour != lasthour) EveryHourUpdate();                                                                            //  if(timeinfo.tm_hour != lasthour) EveryHourUpdate(); 
}
//--------------------------------------------                                                //
...
void EveryHourUpdate(void)
{
 ...
 if (!Mem.StatusLEDOn) SetStatusLED(0,0,0);                                                   // During startup this EveryHourUpdate is run once and will turn the status LEDs of  
 bool ff = NoTextInLeds; 
 ...
 if(timeinfo.tm_hour == Mem.TurnOnLEDsAtHH)
   { LEDsAreOff = false;   lastminute = 99; Displaytime(); Tekstprintln(""); }                // Force a minute update to turn on LEDs
 ...
 SumLDRreadshour  = 0;
 NoofLDRreadshour = 0;                                                                        // and DimLEDs are used to print the time every hour 
...
void InitStorage(void)
{
 ...
 InitLogBuffer();     Tekstprintln("Ringbuffer initialysed"); 
...
// alternative? #define FILENAAM (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : (strrchr(__FILE__, '\\') ? strrchr(__FILE__, '\\') + 1 : __FILE__))   
 ...
 if(!Small) {snprintf(sptext, sizeof(sptext), "Timestamp:%s", 
  ...
  switch(cmd) 
   {
    ...
    case 'F':                                                                                 // Scan WIFI stations
      if(len == 1) 
       {
        CalcDifRTCtimes();  // RTC8
        PrintTimeHMS(1); 
        PrintDifRTCtimes(); // RTC8
    ...
    case 'U':        
       if(len == 2) 
       {
       int rtcIndex = SConstrainInt(InputString, 1, 0, 8);                            // keep the range between 0 and 4; H00 - H04
       SetNTPTimeForRTC(rtcIndex) ;
      }                                                                      // *** Empty *** 
       else snprintf(sptext, sizeof(sptext), "**** Length fault. Enter Ux ****");
       break;
    ...
    case 'Y':   
             // function that will run > 0.5 sec can be placed here                                                                              // *** Empty *** 
       snprintf(sptext, sizeof(sptext), "**** No Use option ****");
      break;
...
{   
 ...
 //Tekstprintlnf("Colour: 0X%08" PRIX32,RGBWColor);
}
//--------------------------------------------                                                //
...
void LEDstartup(uint32_t LEDColour)
{
 ...
 if(ProgressLedNr > NUM_LEDS-1)  ProgressLedNr = NUM_LEDS -1;
}
...
{
    GetTijd(false);  // refreshes timeinfo with local time
...
//--------------------------------------------                                                //
// RTC Fill sptext with time as HMS hh:mm:ss
//--------------------------------------------
...
void WebPage(void) 
{
 ...
 html_info[0] = 0;
 DoNotLog = true;
 SWversion(LastMenuformat);                                                                   // Print the menu to the web page and BLE
 ...
 server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request)                              // Send a GET request to <ESP_IP>/get?input1=<inputMessage>
   { 
    ...
    if ( !inputMessage.equalsIgnoreCase("Y")                                                  // These commands take longer than 1 s to finish 
      && !inputMessage.equalsIgnoreCase("G")                                                  // Do not execute command via ReworkInputMessage
      && !inputMessage.equals("&") )                                                          // But execute in the loop() 
  ...
  for (int i = 0; i < MAX_RTC; i++) 
   {
    ...
    if (!rtc[i].begin()) 
    ...
    else 
     {
      ...
      sprintf(sptext,"RTC%d current time: %02d/%02d/%04d %02d:%02d:%02d ",i, newTime.day(),newTime.month(),newTime.year(),
 ...
 for (int i = 0; i < MAX_RTC; i++)                                                           // Read time of all ACTIVE RTC modules
  {
   ...
   selectTCAChannel(i);                                                                      // Select the RTC channel on the TCA9548A I2C multiplexer
   delay(10);
   DateTime rtcNow = rtc[i].now();
   Tekstprintlnf("RTC %d: %02d/%02d/%04d %02d:%02d:%02d T: %5.2f C",
       i, rtcNow.day(), rtcNow.month(), rtcNow.year(), rtcNow.hour(), rtcNow.minute(), rtcNow.second(), 
       rtc[i].getTemperature());  
  }
}
//--------------------------------------------                                                //
...
void PrintDifRTCtimes(void)
{
 ...
 for (int i = 0; i < MAX_RTC; i++) 
  {  
   ...
   Tekstprintf(";T%d:%2ld",i,RTCdiffSec[i]);
  }
 Tekstprintf(";C: %5.2f", GetMedianRTCTemperature());
}
//--------------------------------------------                                                //
...
void UpdateRTCTimeLEDs(void)
{
 ...
 for (int i = 0; i < MAX_RTC; i++)                                                           // Read time of all ACTIVE RTC modules
  {
   int c;
   if (!rtcActive[i]) c = -1;                                                                // Skip non-active RTC's 
   else c = RTCdiffSec[i]; 
   switch (c)
    {
      case  0 ...  1: ColorLed(i,green);      break;
      case  5 ...  7: ColorLed(i,greenblue);  break;
      case  2 ...  4: ColorLed(i,pink);       break;
      case  8 ... 10: ColorLed(i,darkviolet); break;
      case 11 ... 17: ColorLed(i,purple);     break;
      case 18 ... 25: ColorLed(i,orange);     break;
      case 26 ...999: ColorLed(i,red);        break;
      default:        ColorLed(i,blue); 
    }
  }
SetTempColor(8, timeinfo.tm_hour + 8);      
SetTempColor(9, GetMedianRTCTemperature());                                                     //Last LED number 10 = led 9
ShowLeds(); 
}
...
//-------------------------------------------- 
static time_t To_time_t(const DateTime& dt)
{
 struct tm rtc_tm = {};
 rtc_tm.tm_year  = dt.year()  - 1900;
 rtc_tm.tm_mon   = dt.month() - 1;
 rtc_tm.tm_mday  = dt.day();
 rtc_tm.tm_hour  = dt.hour();
 rtc_tm.tm_min   = dt.minute();
 rtc_tm.tm_sec   = dt.second();
 rtc_tm.tm_isdst = -1;
...
{
 struct tm ntp_tm = timeinfo;
 ntp_tm.tm_isdst = -1;
 const time_t tNTP = mktime(&ntp_tm);
 for (int i = 0; i < MAX_RTC; i++)
   {
    if (!rtcActive[i]) { RTCdiffSec[i] = -1; continue; }
    selectTCAChannel(i);
    delay(10);
    RTCdiffSec[i] = abs(tNTP - To_time_t(rtc[i].now()));
   }
}
...
void selectTCAChannel(uint8_t channel) 
{
  ...
  uint8_t error = Wire.endTransmission();
  if (error != 0)  Tekstprintf("Error selecting channel %d Error code : %d", channel,error);
...
//--------------------------------------------                                                //
// RTC8 set current NTP Time For RTC Number X 
//--------------------------------------------
void SetNTPTimeForRTC(int rtcIndex) 
{
 if (!rtcActive[rtcIndex]) return;
 DateTime referenceTime = GetLocalDateTime() + 1;                                            // Get the current time and add 1 sec setting time 
 selectTCAChannel(rtcIndex);
 delay(50);
 DateTime before = rtc[rtcIndex].now();                                                           // Show time before update
 Tekstprintf("RTC %d: before sync %s", rtcIndex, before.timestamp(DateTime::TIMESTAMP_FULL).c_str());
 rtc[rtcIndex].adjust(referenceTime);                                                             // Update time
 delay(50); 
 DateTime after = rtc[rtcIndex].now();                                                            // Verify
 Tekstprintf("RTC %d:  after sync %s", rtcIndex, after.timestamp(DateTime::TIMESTAMP_FULL).c_str());
...
void PrintTimeForRTC(int rtcIndex) 
{
 ...
 Tekstprintf("RTC %d: %s", rtcIndex, now.timestamp(DateTime::TIMESTAMP_FULL).c_str());  
}
//--------------------------------------------                                                //
...
// Wait for the second tick per RTC to update that RTC 
//--------------------------------------------
void SynchronizeAllRTCs()   // Zoek eerste actieve RTC als referentie
{
  ...
  DateTime referenceTime = GetLocalDateTime() + 1;                                            // Get the current time and add 1 sec setting time  
  Tekstprintln("\n=== RTC SYNCHRONISATION ===");
  Tekstprintlnf("Reference time of NTP RTC %d: %s", referenceRTC, referenceTime.timestamp(DateTime::TIMESTAMP_FULL).c_str());
  for (int i = 0; i < MAX_RTC; i++) 
    {
     if (!rtcActive[i]) continue;
     selectTCAChannel(i);
     time_t t = time(nullptr);                         // Wait for the next clean second boundary
     while (time(nullptr) == t) { delay(1); }         // spin until second ticks over
     DateTime referenceTime = GetLocalDateTime();      // now exactly on the second, no +1 needed
        rtc[i].adjust(referenceTime);
        DateTime after = rtc[i].now();
        Tekstprintlnf("RTC %d: Synced to %s", i, after.timestamp(DateTime::TIMESTAMP_FULL).c_str());
    }
  UpdateRTCTimeLEDs();  
  Tekstprintln("Synchronisation done!\n");
}
 ...
 for (int i = 0; i < MAX_RTC; i++) 
    {
     ...
     Tekstprintlnf("RTC %d: before sync %s", i, before.timestamp(DateTime::TIMESTAMP_FULL).c_str());
     rtc[i].adjust(newTime);
     delay(50);
     DateTime after = rtc[i].now();                                                          // Verify
     Tekstprintlnf("RTC %d: after sync %s", i, after.timestamp(DateTime::TIMESTAMP_FULL).c_str());
    }
 UpdateRTCTimeLEDs();     
 Tekstprintln("All active RTC's updated!\n");
}
//--------------------------------------------                                                //
// RTC8 Set Temperature Color 
// Change color from blue = 7.5C to green 20C to red 32.5C
//--------------------------------------------
void SetTempColor(int Lednr, float temp)
{
 uint8_t T = uint8_t (uint16_t(constrain(temp, 7.5f, 32.5f) * 10) - 75);                             // T (75-325)-75 -> 0<>250                    // 0-255
 uint32_t Red   = (uint32_t)constrain((int)(T - 125) * 2, 0, 255);
 uint32_t Green = (uint32_t)constrain(255 - abs((int)(T - 125) * 2), 0, 255);
 uint32_t Blue  = (uint32_t)constrain((int)(125 - T) * 2, 0, 255);                                                       //
// Tekstprintlnf("Temp= %f T= %d, Red %d, Green %d, Blue %d",temp, T, Red, Green, Blue); 
 ColorLed(Lednr, FuncCRGBW(Red, Green, Blue, 0));
}

//--------------------------------------------                                                //
// RTC8 Get media temperature of all active LEDs
//--------------------------------------------
float GetMedianRTCTemperature(void)
{
 float Temps[MAX_RTC];
 int Count = 0;
 for (int i = 0; i < MAX_RTC; i++)                                                            // Collect temperatures of active RTC's
  {
   if (!rtcActive[i]) continue;
   selectTCAChannel(i);
   Temps[Count++] = rtc[i].getTemperature();
  }
 if (Count == 0) return -273.0;                                                               // No active RTC's
 ...
 if (Count % 2 == 0)                                                                          // Even number: average of two middle values
      return (Temps[Count/2 - 1] + Temps[Count/2]) / 2.0;
 else return  Temps[Count/2];                                                                 // Odd number: middle value
}
