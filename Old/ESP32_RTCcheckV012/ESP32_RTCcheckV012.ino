...
// #define FILENAAM filename_only(__FILE__)
/*
#include <string.h>
#define FILENAAM (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : (strrchr(__FILE__, '\\') ? strrchr(__FILE__, '\\') + 1 : __FILE__))
*/
//------------------------------------------------------------------------------              //
// SPIFFS storage
//--------------------------------------------
...
// sprintf(sptext,"Password: %s", Mem.Password);                                                WTekstprintln(sptext);
 ...
 if(!Small) {sprintf(sptext,"%s",Mem.UseRotary==0 ?"Rotary, keypad and IR-remote OFF":
                        Mem.UseRotary==1 ?"Rotary = ON":
                        Mem.UseRotary==2 ?"Membrane = ON":
                        Mem.UseRotary==3 ?"IR-remote = ON":
                        Mem.UseRotary==4 ?"Ir-remote = ON":"NOP");                                WTekstprintln(sptext); }                           
