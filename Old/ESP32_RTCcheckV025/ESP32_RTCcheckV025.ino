...
{
 temp = constrain(temp, 15.0, 25.0);
 float t = (temp) / 10.0;                         // normalize 0.0 to 1.0
 uint8_t r, g, b;

 if (t < 0.5)                                            // blue to green (15 to 20C)
  {
   float tt = t * 2.0;
   r = 0;
   g = (uint8_t)(255 * tt);
   b = (uint8_t)(255 * (1.0 - tt));
  }
 else                                                     // green to red  (20C to 25C)
  {
   float tt = (t - 0.5) * 2.0;
   r = (uint8_t)(255 * tt);
   g = (uint8_t)(255 * (1.0 - tt));
   b = 0;
  }
 ColorLed(Lednr, ((uint32_t)r << 16) | ((uint32_t)g << 8) | b);
}
