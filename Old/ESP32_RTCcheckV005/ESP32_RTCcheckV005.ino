...
static uint32_t msTickRTC8;                                              // Number of millisecond ticks since we last incremented the second counter
 ...
 for (int i = 0; i < MAX_RTC; i++)                                  // Read time of all ACTIVE RTC modules
  {
   ...
   ColorLed(i,FuncCRGBW( Red,  Green,  Blue, 0));  
  }
ShowLeds(); 
}
...
void PrintDifRTCtimes(void)
{
 ...
 for (int i = 0; i < MAX_RTC; i++) 
  {  
   ...
   sprintf(sptext,"|T%d:%2d",i,RTCdiffSec[i]);
   Tekstprint(sptext);
  }
 Tekstprint("| ");
}
...
void printTimeForRTC(int rtcIndex) 
{
  ...
  sprintf(sptext,"RTC %d: %s", now.timestamp(DateTime::TIMESTAMP_FULL).c_str());
  Tekstprintln(sptext);  
}
//--------------------------------------------                       //
...
void synchronizeAllRTCs()   // Zoek eerste actieve RTC als referentie
{
  ...
  Tekstprintln("\n=== RTC SYNCHRONISATION ===");
    ...
    sprintf(sptext,"Reference time of RTC  %d loops/s",referenceRTC);
  Tekstprintln(sptext); 
