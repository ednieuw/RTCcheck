...
void HandleTekstPrint(AsyncWebServerRequest *request)
{
 ...
 size_t start = (pos + 1) % LogBufferSize;                                                    // Start is the position right after the newline we stopped at
...
{
 temp = constrain(temp, -10.0, 50.0);
 float t = (temp + 10.0) / 60.0;                         // normalize 0.0 to 1.0
 uint8_t r, g, b;

 if (t < 0.5)                                            // blue to green (-10C to 20C)
  {
   float tt = t * 2.0;
   r = 0;
   g = (uint8_t)(255 * tt);
