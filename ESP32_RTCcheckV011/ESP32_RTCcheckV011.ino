/* 
 ...
 Changes V011: T0 is one hour off --> added     rtc_tm.tm_isdst = -1;  
*
********************
How to compile: 
...
Pin Numbering: By GPIO number (legacy). !! With Neopixel not 'By Arduino pin (default)'
...
//--------------------------------------------
//                       #if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL (5, 2, 0)            // NeoPixel is OK since dec 2025 Use EdSoftLED with ESP32 compiler until V3.2.0. Neopixel crashes>
// #define USEEDSOFTLED
//                       #endif
                      #ifdef USEEDSOFTLED
...
/*void CalcDifRTCtimes(void)
{
 struct tm timeinfo_copy = timeinfo;                                 // Make a copy to preserve original timeinfo
 timeinfo_copy.tm_isdst = -1;                                        // Let mktime() determine DST correctly
 time_t tNTP = mktime(&timeinfo_copy);                               // Calculate NTP time ONCE before the loop
 for (int i = 0; i < MAX_RTC; i++) 
   {  
    if (!rtcActive[i]) {RTCdiffSec[i] = -1; continue; }              // -1 is not active
    selectTCAChannel(i);                                             // Select the RTC channel on the TCA9548A I2C multiplexer
    delay(10);
    DateTime now = rtc[i].now();
    struct tm rtc_tm = {};                   // zero-initialize!
    rtc_tm.tm_year = now.year() - 1900;
    rtc_tm.tm_mon  = now.month() - 1;
    rtc_tm.tm_mday = now.day();
    rtc_tm.tm_hour = now.hour();
    rtc_tm.tm_min  = now.minute();
    rtc_tm.tm_sec  = now.second();
    rtc_tm.tm_isdst = -1;                    // KEY FIX
